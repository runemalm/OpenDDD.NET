...
class Writer:
  ...
  def write(self, code):
    ...
    elif (code.what == "aggregate_migrator"):
      self._write_aggregate_migrator(code)
    elif (code.what == "command"):
      self._write_command(code)
    elif (code.what == "entity_id"):
      self._write_entity_id(code)
    elif (code.what == "http_bb_translator"):
      self._write_http_bb_translator(code)
  ...
  def _write_aggregate_migrator(self, code):
    self._write_file(code.path, code.code)
  ...
  def _write_http_bb_translator(self, code):
    self._write_file(code.path, code.code)
